...
{
	public:
		template<typename InitState, typename = typename std::enable_if<std::disjunction<std::is_same<InitState,
		States>...>::value>::type>
		...
		{
			boost::apply_visitor(make_overload_set([&](auto && state) -> decltype(state.handle(event),void()) 
